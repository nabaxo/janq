...
- **KWin Script Recovery**: If janq crashes, gets force-killed, or a KWin script slot gets stuck, toggles silently break and windows stay invisible. janq now has a three-layer recovery system: stale script slots are automatically purged on daemon startup; `--recover` / `-r` CLI flag tells the running daemon to purge scripts, clear caches, and re-grab all windows; and the systray right-click menu has a "Recover" button that does the same thing without needing a terminal. Think of it as janq's "have you tried turning it off and on again" button, except it actually works.
