...
use crate::kwin::{toggle_quake, restore_quake, ensure_grabbed, reset_visibility};
...
pub async fn run_daemon(initial_config: Config, config_path: Option<std::path::PathBuf>, auto_show: bool) -> zbus::Result<()> {
    ...
    std::thread::spawn(move || {
        ...
        if let Some(path) = path_to_watch {
            ...
            if let Some(home) = dirs::home_dir() {
                let path = home.join(".rustake.toml");
