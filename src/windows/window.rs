    ...
    for data in search.found_data {
        ...
        unsafe {
            ...
            if GetWindowRect(hwnd, &mut rect).is_ok() {
                ...
                if (w <= 0 || h <= 0) && !is_minimized { continue; }
                let style = GetWindowLongW(hwnd, GWL_EXSTYLE) as u32;
                if (style & windows::Win32::UI::WindowsAndMessaging::WS_EX_TOOLWINDOW.0) != 0 { continue; }
                let is_visible = IsWindowVisible(hwnd).as_bool();
                let mut score = 0;
                if data.class_name == search.name { score += 5000; }
                else if data.class_name.contains(&search.name) { score += 1000; }
                ...
                if data.class_name == "ime" || data.class_name == "msctfime ui" { score -= 4500; }
                let style_regular = GetWindowLongW(hwnd, windows::Win32::UI::WindowsAndMessaging::GWL_STYLE) as u32;
                if (style_regular & windows::Win32::UI::WindowsAndMessaging::WS_CAPTION.0) != 0 { score += 500; }
                if !data.title.is_empty() { score += 200; }
...
struct MonitorEnumCtx { monitors: Vec<HMONITOR>, }
...
pub async fn toggle_window(app_name: &str, config: &Config) -> bool {
    ...
    let mut cached_hwnd = None;
    {
        ...
        if let Some(h) = cache.get(app_name) {
             unsafe { if windows::Win32::UI::WindowsAndMessaging::IsWindow(h.inner()).as_bool() { cached_hwnd = Some(*h); } }
        }
    }
    let target_hwnd = if let Some(h) = cached_hwnd { h } else {
        ...
        for (name, cfg) in &config.app {
            ...
            let mut cached_h = None;
            {
                ...
                if let Some(h) = cache.get(name) {
                    unsafe { if windows::Win32::UI::WindowsAndMessaging::IsWindow(h.inner()).as_bool() { cached_h = Some(*h); } }
                }
            }
            let found_hwnd = if let Some(h) = cached_h { h } else {
            ...
            if found_hwnd.0 == target_hwnd.0 { continue; }
    ...
    // Abort current animation
    {
        ...
        if let Some(handle) = task_handle.take() { handle.abort(); }
    }

    let mut restore_focus = false;
    {
        let mut v = get_visible_app().write().unwrap();
        if should_show { *v = Some(app_name.to_string()); }
        else {
            *v = None;
            unsafe {
                let fg_window = GetForegroundWindow();
                if fg_window == target_hwnd.inner() { restore_focus = true; }
            }
        }
    }
    ...
    if should_show {
        unsafe {
            ...
            if !fg_window.is_invalid() && fg_window != target_hwnd.inner() { *prev = Some(SendHwnd(fg_window)); }
        }
    }
    ...
    unsafe {
        let monitor = if should_show {
            match config.window.display_mode.as_str() {
                "specific" => {
                    ...
                    if (config.window.display_index as usize) < ctx.monitors.len() { ctx.monitors[config.window.display_index as usize] }
                    else {
                ...
                "active" => {
                    ...
                    if let Some(h) = *prev { MonitorFromWindow(h.0, MONITOR_DEFAULTTONEAREST) }
                    else {
                        let mut cursor_pos = POINT { x: 0, y: 0 };
                        let _ = GetCursorPos(&mut cursor_pos);
                        MonitorFromPoint(cursor_pos, MONITOR_DEFAULTTONEAREST)
        ...
        let prep_layer = |h: HWND| {
            ...
            if (ex & WS_EX_LAYERED.0 as i32) == 0 {
                ...
                let _ = SetWindowPos(h, HWND::default(), 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_FRAMECHANGED);
            }
        };
        ...
        if should_show {
            ...
            let _ = SetWindowPos(target_hwnd.inner(), z_order, target_x, final_target_y, target_w, target_h, SWP_SHOWWINDOW | SWP_NOACTIVATE);
        } else {
...
pub async fn park_window(send_hwnd: SendHwnd, config: &Config, app_cfg: &AppConfig) {
    ...
    unsafe {
        ...
        if (ex_style & WS_EX_LAYERED.0 as i32) == 0 { SetWindowLongW(hwnd, GWL_EXSTYLE, ex_style | WS_EX_LAYERED.0 as i32); }
        ...
        if GetMonitorInfoW(mon, &mut mi).as_bool() {
             ...
             let tw = if w > 0.0 { (if w <= 1.0 { sw as f64 * w } else { w }) as i32 } else { cur_w };
             let th = if h > 0.0 { (if h <= 1.0 { (work_area.bottom - work_area.top) as f64 * h } else { h }) as i32 } else { cur_h };
...
pub fn restore_window_visibility(config: &Config) { for (name, cfg) in &config.app { restore_app_window(name, &cfg.window_class); } }
pub fn reset_visible_app() { let mut v = get_visible_app().write().unwrap(); *v = None; }
